...
export default function Customers() {
    ...
    const isOverdue = (customer) => {
        if (!customer.son_odeme_tarihi) return false
        const balance = getBalance(customer.id)
        if (balance <= 0) return false
        const sonOdeme = new Date(customer.son_odeme_tarihi)
        const bugun = new Date()
        const fark = Math.floor((bugun - sonOdeme) / (1000 * 60 * 60 * 24))
        return fark > 30
    }
    ...
    const getDaysSincePayment = (customer) => {
        ...
        const sonOdeme = new Date(customer.son_odeme_tarihi)
        const bugun = new Date()
        return Math.floor((bugun - sonOdeme) / (1000 * 60 * 60 * 24))
    }
                                        ...
                                        <tr key={c.id} style={{ background: overdue ? '#fff5f5' : 'white' }}>
                                            ...
                                            <td style={{ textAlign: 'center' }}>
                                                <div style={{ display: 'flex', gap: 6, justifyContent: 'center' }}>
                                                    ...
                                                    <button className="btn btn-danger btn-sm" onClick={(e) => remove(e, c.id)}>Sil</button>
                                                </div>
                                            </td>
                                        </tr>
                                    )
